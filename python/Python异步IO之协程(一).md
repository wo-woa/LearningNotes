...
1. 解释了yield和yield from的区别，一个返回完整列表，一个返回迭代器
2. 常用的使用协程的模板，使用wait创建
3. 使用tqdm实现进度条
4. `yield from`后面必须是**子生成器函数**，**而`time.sleep()`并不是生成器**
...
使用`tqdm`实现效果：
![](https://gitee.com/wowoa/typoraPic/raw/master/image/20200503094538.png)
...
![](https://gitee.com/wowoa/typoraPic/raw/master/image/20200503094618.png)
